...
func TestStalledSFTPRequestIsCancelled(t *testing.T) {
	...
	go func() {
		...
		if err != nil { _ = client.Close(); return }
		...
		for {
			...
			for pause.Load() {
				select { case <-stop: return; case <-time.After(5*time.Millisecond): }
			}
			if _, err := server.Write(buffer[:n]); err != nil { return }
		}
	}()
	...
	select {
	case err := <-done:
		if err == nil || ctx.Err() == nil || !remote.IsClosed() { t.Fatalf("stalled request not cancelled: %v", err) }
...
func TestCancelledIdleReaderDoesNotCloseOtherSessions(t *testing.T) {
	...
	if err != nil { t.Fatal(err) }
	cancel()
	if _, err := reader.Read(make([]byte, 4)); !errors.Is(err, context.Canceled) { t.Fatalf("cancelled read: %v", err) }
	_ = reader.Close()
	if remote.IsClosed() { t.Fatal("idle reader cancellation closed an unrelated active endpoint") }
	if _, err := remote.Stat(context.Background(), file); err != nil { t.Fatal(err) }
}
