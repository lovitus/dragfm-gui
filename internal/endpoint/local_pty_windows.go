...
func (l *Local) OpenPTY(ctx context.Context, directory, _ string, rows, columns uint) (PTYSession, error) {
	...
	if err != nil { _ = console.Close(); return nil, err }
	...
	go func() {
		select { case <-ctx.Done(): _ = session.Close(); case <-session.done: }
	}()
	return session, nil
}
...
func (p *windowsPTY) Resize(rows, columns uint) error { return p.console.Resize(uint16(columns), uint16(rows)) }
func (p *windowsPTY) Wait() error { <-p.done; return p.waitErr }
func (p *windowsPTY) Close() error {
	p.once.Do(func() {
		select { case <-p.done: default: _ = p.process.Kill() }
