...
func craftedArchive(t *testing.T, headers ...*tar.Header) []byte {
	...
	for _, header := range headers {
		...
		if header.Typeflag == tar.TypeReg {
			if _, err := tarball.Write(bytes.Repeat([]byte{'x'}, int(header.Size))); err != nil {
				t.Fatal(err)
			}
		}
	}
...
func TestArchiveCannotWriteThroughSymlink(t *testing.T) {
	for _, link := range []string{"absolute", "relative", "root-file"} {
		t.Run(link, func(t *testing.T) {
			...
			if link == "root-file" {
				headers = []*tar.Header{{Name: ".", Typeflag: tar.TypeSymlink, Linkname: outside}, {Name: "pwned", Typeflag: tar.TypeReg, Size: 4}}
			}
			if err := receiveArchive(bytes.NewReader(craftedArchive(t, headers...)), filepath.Join(root, "partial")); err == nil {
...
func TestArchiveRequiresRootAndValidGzipTrailer(t *testing.T) {
	...
	for name, archive := range map[string][]byte{
		...
		t.Run(name, func(t *testing.T) {
			if err := receiveArchive(bytes.NewReader(archive), filepath.Join(t.TempDir(), "partial")); err == nil {
...
func TestArchiveReadOnlyDirectoryRestoredAfterChildren(t *testing.T) {
	...
	if err := receiveArchive(bytes.NewReader(archive), target); err != nil {
		t.Fatal(err)
	}
