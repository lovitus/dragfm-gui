...
func receiveArchiveWithOwnership(reader io.Reader, target string, preserveOwner bool) error {
	...
	if name == "." || name == string(filepath.Separator) {
		return errors.New("invalid archive target")
	}
	...
	if err != nil {
		return fmt.Errorf("archive has no root member: %w", err)
	}
	if first.Name != "." {
		return errors.New("archive must start with exactly one root member")
	}
	var scope *os.Root
	defer func() {
		if scope != nil {
	...
	for {
		...
		if scope == nil {
			return errors.New("non-directory archive root has child members")
		}
		member := filepath.Clean(filepath.FromSlash(header.Name))
		if !filepath.IsLocal(member) || member == "." || seen[member] {
			return fmt.Errorf("invalid or duplicate archive member %q", header.Name)
		}
		seen[member] = true
		...
		for index := 1; index < len(parts); index++ {
			...
			if statErr != nil {
				return fmt.Errorf("archive parent %q: %w", directory, statErr)
			}
			if !info.IsDir() {
				return fmt.Errorf("archive parent %q is not a directory", directory)
			}
		}
		if header.Typeflag == tar.TypeDir {
			if err := scope.Mkdir(member, 0700); err != nil {
				return err
			}
			copy := *header
			copy.Name = member
		...
		if err := extractArchiveMember(scope, member, header, archive, preserveOwner); err != nil {
			return err
		}
	}
	...
	for {
		...
		remaining -= count
		if remaining < 0 {
			return errors.New("excessive archive padding")
		}
	...
	for _, directory := range directories {
		if err := restoreArchiveMetadata(scope, directory.Name, directory, preserveOwner); err != nil {
			return err
		}
	}
	return nil
...
func extractArchiveMember(scope *os.Root, name string, header *tar.Header, reader io.Reader, preserveOwner bool) error {
	...
	case tar.TypeReg, tar.TypeRegA:
		...
		if count != header.Size && copyErr == nil {
			copyErr = io.ErrUnexpectedEOF
		}
		if err := errors.Join(copyErr, file.Sync(), file.Close()); err != nil {
			return err
		}
		return restoreArchiveMetadata(scope, name, header, preserveOwner)
	case tar.TypeSymlink:
		if err := scope.Symlink(header.Linkname, name); err != nil {
			return err
		}
		if preserveOwner {
			return scope.Lchown(name, header.Uid, header.Gid)
		}
		return nil
	default:
...
func restoreArchiveMetadata(scope *os.Root, name string, header *tar.Header, preserveOwner bool) error {
	...
	if preserveOwner {
		if err := scope.Chown(name, header.Uid, header.Gid); err != nil {
			return err
		}
	}
	mode := header.FileInfo().Mode().Perm()
	if preserveOwner {
		mode |= header.FileInfo().Mode() & (os.ModeSetuid | os.ModeSetgid | os.ModeSticky)
	}
	if err := scope.Chmod(name, mode); err != nil {
		return err
	}
