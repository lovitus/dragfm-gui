...
func (s *Session) CallContext(ctx context.Context, action string, options, secret map[string]string) (map[string]string, error) {
	if s == nil { return nil, errors.New("remote helper is not running") }
	select { case <-s.done: return nil, errors.New("remote helper is closed"); default: }
	...
	if s.ctx != nil {
		...
		defer stop()
		if err := s.ctx.Err(); err != nil { return nil, err }
	}
...
func (s *Session) abortTransport() {
	...
	if channel, ok := s.ssh.(interface{ Signal(ssh.Signal) error }); ok { _ = channel.Signal(ssh.SIGTERM) }
	_ = s.ssh.Close()
}
...
func (s *Session) Close() error {
	...
	s.once.Do(func() {
		...
		if s.stdin != nil { stdinErr = s.stdin.Close() }
		if s.remote != nil {
...
type helperOutput struct { mu sync.Mutex; tail []byte }
func (b *helperOutput) Write(data []byte) (int, error) {
	b.mu.Lock(); defer b.mu.Unlock()
	const limit = 16 << 10
	n := len(data)
	if n >= limit { b.tail = append(b.tail[:0], data[n-limit:]...); return n, nil }
	if len(b.tail)+n > limit { b.tail = b.tail[len(b.tail)+n-limit:] }
